...
module.exports = function(grunt) {
	grunt.initConfig({
		...
		clean: {
			build: ['dist'],
			cleanup_js: ['dist/js/*.*', '!dist/js/symstats.*'],
			cleanup_css: ['dist/css/*.css', '!dist/css/symstats.*.css'],
			build_lite: ['dist-lite'],
			cleanup_js_lite: ['dist-lite/js/*.*', '!dist-lite/js/symstats.*'],
			cleanup_css_lite: ['dist-lite/css/*.css', '!dist-lite/css/symstats.*.css']
		},
		jade: {
			build: {
				options: {
		...
		concat: {
			...
			symstats: {
				...
				dest: 'dist/js/symstats.min.js',
				nonull: true
			},
			css: {
				src: ['dist/css/*.min.css', 'dist/css/*.css'],
				dest: 'dist/css/symstats.min.css'
			},
			vendor_lite: {
				options: {
					sourceMap: false,
			...
			symstats_lite: {
				...
				dest: 'dist-lite/js/symstats.min.js',
				nonull: true,
			},
			css_lite: {
				src: ['dist-lite/css/*.min.css', 'dist-lite/css/*.css'],
				dest: 'dist-lite/css/symstats.min.css'
			}
		},
		uglify: {
			app: {
	...
	grunt.registerTask('default', ['clean:build', 'clean:cleanup_js', 'clean:cleanup_css', 'jade:build', 'copy:build', 'cssmin:build', 'concat:vendor', 'concat:scripts', 'uglify:app', 'concat:symstats', 'concat:css', 'clean:cleanup_js', 'clean:cleanup_css']);
	grunt.registerTask('lite', ['clean:build_lite', 'clean:cleanup_js_lite', 'clean:cleanup_css_lite', 'jade:build_lite', 'copy:build_lite', 'cssmin:build_lite', 'concat:vendor_lite', 'concat:scripts_lite', 'uglify:app_lite', 'concat:symstats_lite', 'concat:css_lite', 'clean:cleanup_js_lite', 'clean:cleanup_css_lite']);
