...
angular.module('symStatsApp.filters', [])
...
.filter('primaryFilter', ['$sce', '$filter', function($sce, filter) {
	return function(block, symbase) {
		if( block.dminer.replace(/0x/,"") != symbase.replace(/0x/,"") )
			return $sce.trustAsHtml('<i class="icon-cancel"></i>');
		else
			return $sce.trustAsHtml('<i class="icon-check"></i>');
	};
}])
.filter('warrantFilter', ['$sce', '$filter', function($sce, filter) {
	return function(warrant) {
		if( warrant == "" || !warrant ){
			return $sce.trustAsHtml('<i class="icon-cancel"></i>');
		}else{
			if( warrant == "A"){
				return $sce.trustAsHtml('<span class="badge badge-primary">'+warrant+'</span>');
			}else{
				return $sce.trustAsHtml('<span class="badge badge-info">'+warrant+'</span>');
			}
		}
	};
}])
...
.filter('gasPriceFilter', ['$filter', function(filter) {
	...
	return function(price) {
		...
		if(price.length < 4)
			return numberFilter(price) + " hug";

		if(price.length < 7)
			return numberFilter(price/1000) + " khug";

		if(price.length < 10)
			return numberFilter(price/1000000) + " mhug";

		if(price.length < 13)
			return numberFilter(price/1000000000) + " ghug";

		return numberFilter(price.substr(0, price.length - 18)) + " sym";
	}
}])
...
.filter('verifierTextFilter', ['$sce', '$filter', function($sce, filter) {
	return function(verifiers) {
		var html = "";
		for(var i in verifiers){
			html += '<span class="verifier-item">'+ verifiers[i] +'</span>'
			if(i >= 5){
				break;
			}
		}
...
.filter('primaryNodeFilter', function() {
	return function(primaryNode) {
		return primaryNode
	}
})
.filter('primaryNodeClass', function() {
	return function(primaryNode) {

		if(primaryNode == "Unknown")
			return 'text-warning';
