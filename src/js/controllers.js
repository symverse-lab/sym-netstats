...
symStatsApp.controller('StatsCtrl', function($scope, $filter, $localStorage, socket, _, toastr) {
	...
	$scope.nodesActive = 0;
	$scope.bestBlock = 0;
	$scope.bestCitizenBlock = 0;
	$scope.bestWarrantBlock = 0;
	$scope.lastBlock = 0;
	$scope.upTimeTotal = 0;
	...
	$scope.primaryNode = "";
	$scope.verifiers = [];
	$scope.verifiersCount = 0;
	$scope.bestBlockDateTime = 0;
	$scope.currentWarrantBlock = {};
	...
	$scope.gasLimit = _.fill(Array(MAX_BINS), 2);
	$scope.miners = [];
		...
		switch(action)
		{
			...
			case "update":
				...
				if( index >= 0 && !_.isUndefined($scope.nodes[index]) && !_.isUndefined($scope.nodes[index].stats) )
				{
					...
					updateBestBlock();
					updateBestBlockPrimaryAndVerifier();
				}
			...
			case "wblock":
				...
				if( !_.isUndefined(data.id) && index >= 0 ) {
					if( !_.isUndefined(node) && !_.isUndefined(node.stats) ) {
						$scope.currentWarrantBlock = data.wblock
					}
				}
			...
			case "stats":
				...
				if( !_.isUndefined(data.id) && index >= 0 )
				{
					...
					if( !_.isUndefined(node) && !_.isUndefined(node.stats) )
					{
						...
						$scope.nodes[index].stats.warrant = data.stats.warrant;
			...
			case "tpsstatus":
				if( !_.isUndefined(data.id) && !_.isUndefined(data.tpsstatus) )
				{
					if( !_.isUndefined(data.tpsstatus.tps) && !_.isUndefined(data.tpsstatus.txs) && !_.isUndefined(data.tpsstatus.bgt))
					{
						$scope.tps = data.tpsstatus.tps;
						$scope.txs = data.tpsstatus.txs;
						$scope.bgt = data.tpsstatus.bgt;
					}
				}
			break;
			...
			case "charts":
				...
				if( !_.isEqual($scope.gasLimit, data.gasLimit) && data.gasLimit.length >= MAX_BINS )
					$scope.gasLimit = data.gasLimit;
	...
	function updateBestBlockPrimaryAndVerifier()
	{
		if( $scope.nodes.length )
		{
			...
			var gasVerifier = _( bestBlock.verifiers )
			.map(function (item)
			{
				return item.SymID;
			})
			...
			var primaryNode = _.filter($scope.nodes, function (node)
			{
				return node.stats.symbase == bestBlock.dminer
			})
			$scope.primary = bestBlock.dminer;
			if(primaryNode && primaryNode.length > 0){
				$scope.primaryNode = primaryNode[0].id;
			}else{
				$scope.primaryNode = "Unknown";
			}
			$scope.verifiers = gasVerifier;
			$scope.verifiersCount = gasVerifier.length;
			/*
			$scope.bestBlockDateTime = 0;*/
		}
	}
	...
	function updateBestBlock()
	{
		if( $scope.nodes.length )
		{
			...
			var bestWBlock = _.max($scope.nodes, function (node) {
				return parseInt(node.stats.block.wbNum);
			}).stats.block.wbNum;

			var bestCNumber = _.max($scope.nodes, function (node) {
				return parseInt(node.stats.block.cbNum);
			}).stats.block.cbNum;

			if( bestBlock !== $scope.bestBlock )
			{
				$scope.bestBlock = bestBlock;
				$scope.bestWarrantBlock = bestWBlock;
				$scope.bestCitizenBlock = bestCNumber;
