...
api.on('connection', function (spark)
{
	...
	spark.on('hello', function (data)
	{
		...
		if( !_.isUndefined(data.id) && !_.isUndefined(data.info) )
		{
			...
			data.ip = data.ip.replace( /::ffff:/gi, '');
			data.spark = spark.id;
	...
	spark.on('wblock', function (data)
	{
		if( !_.isUndefined(data.id) && !_.isUndefined(data.wblock) )
		{
			Nodes.setWarrant(data.id, data.wblock, function (err, stats)
			{
				if(err !== null)
				{
					console.error('API', 'WBK', 'Warrant Block error:', err);
				}
				else
				{
					if(stats !== null)
					{
						...
						console.success('API', 'WBK', 'Warrant Block:', data.wblock['warrantNumber'], 'from:', data.id);
					}
				}
			});
		}
		...
		{
			console.error('API', 'WBK', 'Block error:', data);
		}
	});
	...
	spark.on('tpsstatus', function (data)
	{
		if( !_.isUndefined(data.id) && !_.isUndefined(data.tpsstatus) )
		{
			Nodes.updatetpsstatus(data.id, data.tpsstatus, function (err, tpsstatus)
			{
				if(err !== null)
				{
					console.error('API', 'TPS', 'Tps Status error:', err);
				}
				else
				{
					if(tpsstatus !== null)
					{
						...
						console.success('API', 'TPS', 'Tps', data.tpsstatus['tps'], 'Txs', data.tpsstatus['txs'], 'Bgt(ms)', data.tpsstatus['bgt'] ,'from:', data.id);
						// console.success('API', 'TPS', 'Tps Status from:', data.id);
					}
				}
			});
		}
		...
		{
			console.error('API', 'TPS', 'Tps Status error:', data);
		}
	});
