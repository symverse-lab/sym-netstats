...
Node.prototype.getInfo = function(callback)
{
	return {
		...
		stats: {
			...
			symbase: this.stats.symbase,
			primary: this.stats.primary,
			warrant: this.stats.warrant,
			...
			tpsstatus: this.stats.tpsstatus,
		},
...
Node.prototype.setBasicStats = function(stats, callback)
{
	if( !_.isUndefined(stats) )
	{
		...
		}) )
		{
			...
			this.stats.primary = (!_.isUndefined(stats.primary) ? stats.primary : false);
			this.stats.warrant = (!_.isUndefined(stats.warrant) ? stats.warrant : "");
			this.stats.symbase = stats.symbase;
...
Node.prototype.setTpsStatus = function(tpsstatus, callback)
{
	if( !_.isUndefined(tpsstatus) )
	{
		if( !_.isEqual(tpsstatus, this.stats.tpsstatus) )
		{
			this.stats.tpsstatus = tpsstatus;
	...
	{
		callback('tpsstatus undefined', null);
	}
}
