...
History.prototype.add = function(block, id, trusted, addingHistory)
{
	...
	if( !_.isUndefined(block) && !_.isUndefined(block.number)  && !_.isUndefined(block.transactions)  && block.number > 0 )
	{
		...
		else
		{
			...
			if( this._items.length === 0 || (this._items.length === MAX_HISTORY && block.number > this.worstBlockNumber()) || (this._items.length < MAX_HISTORY ) )
			{
...
History.prototype.getBestPrimaryAndVerifier = function()
{
	var bestBlock = this.bestBlock()
